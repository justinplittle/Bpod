...
%            else
     ...
     case 'get_sound_names',   %  -------- CASE GET_SOUND_ID -------------------
       id = arg1;
       
       if strcmp(id,'all')
           out=the_sounds(:,nameCol);
       else
           rownum = find(strcmp(the_sounds(2:nsounds+1,1), id));
           if isempty(rownum),
               error('No sound with id %s declared yet', id);
           else
               out = the_sounds{rownum+1, nameCol};
           end;
       end
