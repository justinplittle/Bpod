% [x, y] = SPSinterface(obj, action, softpokestay_name, x, y)
%
...
%
%   [x, y] = SPSinterface(obj, 'add', sps_name, x, y)
%
...
%
%   sma = SPSinterface(obj, 'add_sma_states', sps_name, sma, ... 
...
function [varargout] = SPSinterface(obj, action, pname, x,y , varargin)
...
%% add_sma_states
  ...
  case 'add_sma_states'
    if nargin < 4, error('SPS:nargin', 'action=add_sma_states requires sps_name and sma as next arguments'); end;
    sma   = x;
    if ~isa(sma, 'StateMachineAssembler'), error('SPS:badargs', 'action=add_sma_states requires the 4th arg to be a @StateMachineAssembler object'); end;
    ...
    if isempty(success_exitstate_name) ||  isempty(abort_exitstate_name), 
      error('SPS:badargs', ['action=add_sma_states requires the name-value pairs ''success_exitstate_name''\n' ...
        'and ''abort_exitstate_name'' to be given explicit defined values-- default values not legal.']);
    end;
    if ~ismember(pokeid, {'C' 'L' 'R'}),
      error('SPS:badargs', 'pokeid must be one of ''C'' ''L'' or ''R''');
    end;
    ...
    sma = add_state(sma, ... %NoseInDoff
      ...
      Sound1_In,    'current_state+6' , ... %NoseInDoffSound1Trig   (7)
      Sound2_In,    'current_state+7' });   %NoseInDoffSound2Trig   (8)
    
    % -- 2) Start Nose Out, Doff  ---                                        StartNoseOutDoff (2)
    sma = add_state(sma, 'self_timer', 0.0001, ...
      ...
      Sound1_In,    'current_state+9' , ... %NoseOutDoffSound1Trig  (11)
      Sound2_In,    'current_state+10'});   %NoseOutDoffSound2Trig  (12)

    % -- 3) Nose Out, Doff  ---                                              NoseOutDoff(3)
    sma = add_state(sma, ...
      'input_to_statechange', { ...
      ...
      Sound1_In,    'current_state+8' , ... %NoseOutDoffSound1Trig  (11)
      Sound2_In,    'current_state+9' });   %NoseOutDoffSound2Trig  (12)
        
    % -- 4) NoseIn, Don ---                                                  NoseInDon(4)
    sma = add_state(sma, ... %NoseInDon
      ...
      Sound1_In,    'current_state+5' , ... %NoseInDonSound1Trig    (9)
      Sound2_In,    'current_state+6' });   %NoseInDonSound2Trig    (10)
    
    % -- 5) Start Nose Out, Don  ---                                         StartNoseOutDon(5)
    sma = add_state(sma, 'self_timer', 0.0001, ... %StartNoseOutDon
      ...
      Sound1_In,    'current_state+8' , ... %NoseOutDonSound1Trig   (13)
      Sound2_In,    'current_state+9' });   %NoseOutDonSound2Trig   (14)

    % -- 6) Nose Out, Don  ---                                               NoseOutDon(6)
    sma = add_state(sma, ... %NoseOutDon (6)
      ...
      Sound1_In,    'current_state+7' , ... %NoseOutDonSound1Trig   (13)
      Sound2_In,    'current_state+8' });   %NoseOutDonSound2Trig   (14)
    
    % ---------------
    % -- 7) Nose In, DOut off Sound 1 Trig                                   NoseInDoffSound1Trig (7)
    sma = add_state(sma, 'self_timer', 0.0001, ... %NoseInDoffSound1Trig (7)
      ...
      DOutWave_In,  'current_state-3', ... %NoseInDon               (4)
      Sound2_In,    'current_state+1'});   %NoseInDoffSound2Trig    (8)

    % -- 8) Nose In, DOut off Sound 2 Trig                                   NoseInDoffSound2Trig (8)
    sma = add_state(sma, 'self_timer', 0.0001, ... %NoseInDoffSound2Trig (8)
      ...
      Sound1_In,    'current_state-1' });   %NoseInDoffSound1Trig   (7)
    
    % ---------------
    % -- 9) Nose In, DOut on Sound 1 Trig                                    NoseInDonSound1Trig (9)
    sma = add_state(sma, 'self_timer', 0.0001, ... %NoseInDonSound1Trig (9)
      ...
      Sound2_In,    'current_state+1' });   %NoseInDonSound2Trig    (10)

    % -- 10) Nose In, DOut on Sound 2 Trig                                   NoseInDonSound2Trig (10)
    sma = add_state(sma, 'self_timer', 0.0001, ... %NoseInDonSound2Trig (10)
      ...
      Sound1_In,    'current_state-1' });   %NoseInDonSound1Trig    (9)

    
    % ---------------
    % -- 11) Nose Out, Doff  Sound 1 Trig ---                                NoseOutDoffSound1Trig (11)
    sma = add_state(sma, 'self_timer', 0.0001, ... %NoseOutDoffSound1Trig (11)
      'output_actions', {'SoundOut', Sound1Id}, ...
      'input_to_statechange', { ...
      ...
      Sound2_In,    'current_state+1' });   %NoseOutDoffSound2Trig  (12)

    % -- 12) Nose Out, Doff  Sound 2 Trig ---                                NoseOutDoffSound2Trig (12)
    sma = add_state(sma, 'self_timer', 0.0001, ... %NoseOutDoffSound2Trig (12)
      'output_actions', {'SoundOut', Sound2Id}, ...
      'input_to_statechange', { ...
      ...
      Sound1_In,    'current_state-1' });   %NoseOutDoffSound1Trig  (11)

    % ---------------
    % -- 13) Nose Out, Don  Sound 1 Trig ---                                 NoseOutDonSound1Trig (13)
    sma = add_state(sma, 'self_timer', 0.0001, ... %NoseOutDonSound1Trig (13)
      'output_actions', {'SoundOut', Sound1Id, 'DOut', DOut}, ...
      'input_to_statechange', { ...
    ...
    sma = add_state(sma, 'self_timer', 0.0001, ... %NoseOutDonSound2Trig (14)
      ...
      Sound1_In,    'current_state-3' });   %NoseOutDoffSound1Trig  (11)
  ...
  case 'set',
    ...
    while ~isempty(varargin),
      if length(varargin)<2,
        error('SPSinterface:BadArgs', '"set" needs name-value pairs');
      end;
      ...
      try
        ...
        warning('SPS:Not_found', 'Couldn''t find parameter named %s, nothing changed', [pname '_' param]);
        return;
      end;
      sph.value = newvalue;
      callback(sph);
...
%% get
  ...
  case 'get',
    ...
    try
      ...
      warning('SPS:Not_found', 'Couldn''t find parameter named %s, returning NaN', [pname '_' param]);
      varargout{1} = NaN;
    end;
...
%% close_all    
  ...
  otherwise,
    warning('SPSinterface:Invalid', 'Don''t know action "%s", doing nothing', action);
